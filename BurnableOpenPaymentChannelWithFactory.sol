...
//
// the seller can withdraw profits as long as their balance remains >= the buyer's.
//
// the seller can close the channel at any time.
// the buyer can close the channel when there is no payment request pending.
//  
// after the channel has closed, both parites can withdraw their share of ether in the account.
//
...
contract paymentChannel {
    ...
    event ChannelClosed();
    event ProfitWithdrawnBySeller(uint Amount);
    event ShareCollected(uint AmountWithdrawn);
    event FundsAdded(uint AmountAdded);

    function paymentChannel(address _seller, bytes8 _data, uint _firstPayment)
        payable
    {
        seller = _seller;
   ...
   function addFunds()
    ...
    function withdrawProfit(uint _amount)
        onlySeller
        inState(State.Active)
        condition(sellerBalance - _amount >= buyerBalance)
    {
        sellerBalance -= _amount;
        if (!seller.send(_amount)) throw;
        ProfitWithdrawnBySeller(_amount);
    }
...
contract paymentChannelFactory {
    event PaymentChannelCreated(address NewChannelAddress);

    function newPaymentChannel(bytes8 _data, uint _firstPayment)
        public
        payable
        returns (address)
    {
        address _seller = msg.sender;
        address newChannelAddress = (new paymentChannel).value(msg.value)(_seller, _data, _firstPayment);
        PaymentChannelCreated(newChannelAddress);
